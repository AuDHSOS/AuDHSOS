...
everything in here; the other waits on the interrupt, drains the receive
FIFO, and sends the bytes to the first over the same endpoint as one drain
message: `encode_drain` writes it, `Console::take_drain` reads it. Nothing is
