...
split, merge, close, and reference count — so the whole policy runs on the
...
when exclusive ownership comes back (D-12). A returned object with foreign
handles or mappings stays retired: it is neither zeroed nor reallocated.
Allocation rechecks retired objects through `memory_references`; only one
remaining reference, the server's own handle, permits reclamation.
Two passes over the same bytes, and both
