...
The two programs of the network: `server-net`, which drives the virtio
network device and answers the socket protocol, and `app-net`, which uses
a socket.

They are a package of their own and not two more binaries of
...
What the two share with every other program — the client of the memory
