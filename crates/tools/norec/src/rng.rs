...
//!
//! The same seed yields the same test case, which is what makes a finding
//! reproducible from the seed alone. It is the generator of `test-support`,
//! written again here because that crate is a dev-dependency and this is
//! product code.

/// A deterministic pseudo-random generator.
#[derive(Clone, Debug)]
pub(crate) struct Rng {
    /// The state the next draw advances.
    state: u64,
}
...
impl Rng {
    ...
    /// A uniform value in `low..=high`, in either order.
    pub(crate) fn range(&mut self, low: u64, high: u64) -> u64 {
    ...
    /// A uniform index into `len` items, or `None` for none.
    pub(crate) fn index(&mut self, len: usize) -> Option<usize> {
        if len == 0 {
            return None;
        }
        usize::try_from(self.below(u64::try_from(len).unwrap_or(u64::MAX))).ok()
    }
