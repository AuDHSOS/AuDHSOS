// SPDX-License-Identifier: AGPL-3.0-only
// Copyright (C) 2026 Manuel Baesler and contributors

//! AES against the vectors of FIPS 197, appendices B and C.

use crate::aes::{Aes, LANES, sub_word};
use crate::tests::{hex, unhex};

/// The substitution box of FIPS 197, figure 7.
const SBOX: [u8; 256] = [
    ,
];

#[test]
fn the_substitution_box_matches_the_published_table() {
    for value in 0u8..=255 {
        let substituted = sub_word([value, 0, 0xFF, 0x53]);
        let expected = [SBOX[usize::from(value)], SBOX[0], SBOX[255], SBOX[0x53]];
        assert_eq!(substituted, expected, "input {value:#04x}");
    }
}

#[test]
fn the_first_appendix_vector_encrypts_as_documented() {
    let key = unhex("2b7e151628aed2a6abf7158809cf4f3c");
    let mut block = [0u8; 16];
    for (slot, byte) in block
        .iter_mut()
        .zip(unhex("3243f6a8885a308d313198a2e0370734"))
    {
        *slot = byte;
    }
    let mut material = [0u8; 16];
    for (slot, byte) in material.iter_mut().zip(key) {
        *slot = byte;
    }
    Aes::new_128(&material).encrypt_block(&mut block);
    assert_eq!(hex(&block), "3925841d02dc09fbdc118597196a0b32");
}

#[test]
fn the_key_length_vectors_encrypt_as_documented() {
    let plaintext = unhex("00112233445566778899aabbccddeeff");
    let mut short = [0u8; 16];
    for (slot, byte) in short.iter_mut().zip(0u8..) {
        *slot = byte;
    }
    let mut long = [0u8; 32];
    for (slot, byte) in long.iter_mut().zip(0u8..) {
        *slot = byte;
    }

    let mut block = [0u8; 16];
    for (slot, byte) in block.iter_mut().zip(plaintext.iter()) {
        *slot = *byte;
    }
    Aes::new_128(&short).encrypt_block(&mut block);
    assert_eq!(hex(&block), "69c4e0d86a7b0430d8cdb78070b4c55a");

    let mut block = [0u8; 16];
    for (slot, byte) in block.iter_mut().zip(plaintext.iter()) {
        *slot = *byte;
    }
    Aes::new_256(&long).encrypt_block(&mut block);
    assert_eq!(hex(&block), "8ea2b7ca516745bfeafc49904b496089");
}

#[test]
fn four_lanes_encrypt_what_four_single_blocks_encrypt() {
    let mut key = [0u8; 16];
    for (slot, byte) in key.iter_mut().zip(0u8..) {
        *slot = byte;
    }
    let cipher = Aes::new_128(&key);

    let mut blocks = [[0u8; 16]; LANES];
    for (group, block) in blocks.iter_mut().enumerate() {
        for (lane, byte) in block.iter_mut().enumerate() {
            *byte = u8::try_from(group.wrapping_mul(16).wrapping_add(lane)).unwrap_or(0);
        }
    }
    let singles = blocks;
    cipher.encrypt_blocks(&mut blocks);

    for (group, single) in singles.iter().enumerate() {
        let mut alone = *single;
        cipher.encrypt_block(&mut alone);
        assert_eq!(blocks.get(group), Some(&alone), "lane group {group}");
    }
}

#[test]
fn clearing_a_key_leaves_no_round_key_to_tell_two_ciphers_apart() {
    let mut first = Aes::new_128(&[0x11u8; 16]);
    let mut second = Aes::new_128(&[0x22u8; 16]);
    let mut block = [0x33u8; 16];
    let mut other = block;
    first.encrypt_block(&mut block);
    second.encrypt_block(&mut other);
    assert_ne!(hex(&block), hex(&other), "different keys before the clear");

    first.clear();
    second.clear();
    let mut block = [0x33u8; 16];
    let mut other = block;
    first.encrypt_block(&mut block);
    second.encrypt_block(&mut other);
    assert_eq!(hex(&block), hex(&other), "the round keys are gone");
}

#[test]
fn clearing_a_256_bit_key_leaves_no_round_key_either() {
    let mut first = Aes::new_256(&[0x44u8; 32]);
    let mut second = Aes::new_256(&[0x55u8; 32]);
    first.clear();
    second.clear();
    let mut block = [0x66u8; 16];
    let mut other = block;
    first.encrypt_block(&mut block);
    second.encrypt_block(&mut other);
    assert_eq!(hex(&block), hex(&other));
}
