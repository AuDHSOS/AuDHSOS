...
95. The order `PRAGMA integrity_check` writes its problems in, and the
    ...
    records.
96. Every database `PRAGMA integrity_check` walks, and the one row the
...
207. The names a `RETURNING` answers its columns under, and the
     `TABLE.*` it takes none of. Built, which D-507 records.
208. The statements written inside a `RETURNING`. Built, which D-508
     records.
...
221. The place a parameter of the harness stands at. Built, which D-521
     records.
...
225. The rows a `DROP TABLE` takes away before the table goes, and what
     their keys read of a schema that no longer answers them. Built,
     which D-525 records.
...
under the `tclsh` of the machine. Of 123 553 cases in 844 files, 118 868
pass, 2490 answer differently, and 2195 name something the engine
