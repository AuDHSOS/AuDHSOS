...
| `eval`, `func`, `agg` | An expression over a row; forty-six scalar functions; seven aggregates; the four shapes of statement an expression uses. | D-143, D-144, D-148, D-154, D-160, Q5 |
...
| Expressions, answered | 17154 | recorded oracle, `eval.corpus` |
...
| 3 | Writing: the b-tree writer, transactions, the journal in four modes, the WAL. `changes`, `total_changes` and `last_insert_rowid` answer nought until then, which is what a connection that has written nothing answers. | Q7 |
...
| 7 | `format` and `printf`, which are a format language of their own, and `unistr`. | Q8 |
