...
[document 14](../14-secure-shell-as-a-client.md), section 14.5, that no
standards body published. It was specified in a file of the OpenSSH
source, and OpenSSH has since replaced that file with a pointer to an
IETF draft. Both documents are here, verbatim and with their checksums,
so that a constant can be checked against its source without a network
and so that the source cannot change under the crate that cites it. This
is [`docs/rfc/`](../rfc/README.md) for a body that is not a standards
body, under the same rule and for the same reason (D-59, D-100, D-124).
...
## Why the OpenSSH file is a revision and not the current one
...
## What the client takes from them
...
The OpenSSH file is part of a distribution whose `LICENCE` summarizes
itself as "all components are under a BSD licence, or a licence more
free than that". The file carries no notice of its own, and nothing in
those terms restricts a verbatim copy.
...
Both are therefore the first case of D-124: documents whose licence
permits the copy.
