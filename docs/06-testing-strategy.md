...
- The watchdog's decision: a run that has not begun and a run without a
  ...
  ended.
- The coverage table (D-140): a feature nothing reached is claimed and is
...
- The protocol (D-140): every message of either side survives the wire; a
...
- The orchestrator (D-140): the corpus is dealt round robin and read once;
