...
without a network: with `ScriptedRng` supplying the client's private key
and random, every derived secret, every handshake message, and every
record on the wire must match the document byte for byte. That trace uses an
