...
| G6b | Die Methoden aus 23.1.3, die in das Script zurückrufen: `map`, `filter`, `forEach`, `every`, `some`, `reduce`, `reduceRight` und `sort`. Der Zustand des Laufs überlebt die Frames der Callbacks, gehört also nicht in den Frame des Aufrufers, sondern in ein Objekt des Heaps, das der Collector verfolgt, gehalten von einem Root, den `Resume` nennt. Zwei Dinge fehlen dafür noch, und sie sind der eigentliche Inhalt dieses Schritts: eine resumable Native braucht einen eigenen Frame, weil der Callback drei Argument-Register (Element, Index, Array) braucht, die die Aufrufstelle des Scripts nie belegt hat; und Argumentwerte dürfen nicht in einem `Call` über `bind_this` hinweg getragen werden, weil das allozieren kann und die Referenzen dann veraltet sind. Beides ist erst zu bauen, danach die acht Methoden auf einem Mechanismus. | `[1,2,3].map(function(x){return x*2})` antwortet wie im Stack-Backend. |
...
| G6d | Die Konstruktoren, an denen die Varianten jetzt stehen: `%Number%` (21.1), `%Boolean%` (20.3), dazu `%Reflect%` (28.1) und `%Symbol%` (20.4). | Die Varianten, die heute an einem dieser Namen stehen, erreichen ihre Assertions. |
