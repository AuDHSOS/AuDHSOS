...
| G5u | `Function.prototype.bind` (20.2.3.2) und das Bound Function Exotic Object (10.4.1). `harness/propertyHelper.js` scheitert daran, und mit ihm 946 Varianten, die heute als fehlgeschlagener Harness zählen. | `harness/propertyHelper.js` läuft im Engine-Core durch. | `%Function%` (20.2) auf dem globalen Objekt, und `%Function.prototype%.call` und `.bind` (20.2.3.3, 20.2.3.2), die der Harness braucht, um eine Methode vom Receiver zu lösen. `harness/propertyHelper.js` scheitert daran, und mit ihm 946 Varianten, die heute als fehlgeschlagener Harness zählen. | `harness/propertyHelper.js` läuft im Engine-Core durch. |
