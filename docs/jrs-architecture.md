...
| G5m | Ein Property-Write unter einem Key, den das Lowering nicht als Number typisieren kann (13.15.2, `PutValue` 6.2.5.5, `OrdinarySetWithOwnDescriptor` 10.1.9.2). Gelesen wird unter jedem Key, geschrieben nur unter einem statischen Namen, weil ein String-Key einen Setter eines Prototypes nennen könnte, den dieser Realm nicht gebaut hat. Die Lücke gehört zur Laufzeit, wie beim Lesen. `harness/propertyHelper.js` scheitert in `isWritable` daran, und mit ihm 946 Varianten, die heute als fehlgeschlagener Harness zählen. | `harness/propertyHelper.js` läuft im Engine-Core. |
