...
| G5n | Ein Object, das nur ein Zweig macht: 14.6.2 führt die beiden Zustände eines `if` zusammen, und ein Layout, das die eine Seite trägt und die andere nicht, ist ein Unterschied, den der Join noch nicht nimmt. `harness/propertyHelper.js` scheitert in `verifyCallableProperty` daran, und mit ihm 946 Varianten, die heute als fehlgeschlagener Harness zählen. | `harness/propertyHelper.js` läuft im Engine-Core. |
