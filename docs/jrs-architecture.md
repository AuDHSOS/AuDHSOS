...
| G6b | Die Methoden aus 23.1.3, die in das Script zurückrufen: `map`, `filter`, `forEach`, `every`, `some`, `reduce`, `reduceRight` und `sort`. Der Core muss dafür aus einer Native heraus eine Funktion des Scripts betreten und dort fortsetzen, wo er aufgehört hat, wie es 7.1.1 für ein `valueOf` schon tut. | `[1,2,3].map(function(x){return x*2})` antwortet wie im Stack-Backend. |
